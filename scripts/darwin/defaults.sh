...
check_fda() {
  local test_file="$HOME/Library/Application Support/com.apple.TCC/TCC.db"
  if ! test -w "$test_file"; then
    return 1
  fi
}

if ! check_fda; then
  echo "Your terminal doesn't have Full Disk Access. Bailing out."
  return 1
fi
...
defaults write -g AppleLanguages -array "en" "es" "fr"
